from flask import Flask, render_template, request, Response, redirect, flash, url_for, jsonify  # Importing Flask and other packages like redirects, flash messages, etc
...
# constrcturs UniProt URL with gene name
    uniprot_api = f"https://rest.uniprot.org/uniprotkb/search?query={gene_name}+AND+organism_id:9606&fields=accession&format=json"
...
def save_feedback():
    ...
    phenotypes = request.form.get('phenotypes')
    snps = request.form.get('snps')
    improvements = request.form.get('improvements')
    satisfaction = request.form.get('satisfaction')
    
    # Process the feedback and save it to the feedback.txt file
    with open('feedback.txt', 'a') as f:
        f.write(f'Phenotypes: {phenotypes}\n')
        f.write(f'SNPs: {snps}\n')
